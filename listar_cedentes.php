...
$total_results = 0;
try {
    $countSql = "SELECT COUNT(id) FROM cedentes $whereSql";
    $stmtCount = $pdo->prepare($countSql);
    $stmtCount->execute($params_count);
    $total_results = (int)$stmtCount->fetchColumn();
} catch (PDOException $e) {
    echo "<div class='alert alert-danger'>Erro Crítico [Count Cedentes]: " . htmlspecialchars($e->getMessage()) . "</div>";
    $cedentes = [];
    $total_pages = 0;
}
...
$cedentes = []; // Inicializa
try {
    $sql = "SELECT id, empresa as nome, email, telefone, empresa, tipo_pessoa, documento_principal FROM cedentes
            $whereSql
    ...
    echo "<div class='alert alert-danger'>Erro Crítico [Data Cedentes]: " . htmlspecialchars($e->getMessage()) . "</div>";
    // $cedentes já está vazio
}
...
<head>
    ...
    <title>Gerenciar Cedentes</title>
...
<body>
    ...
    <div class="container-fluid px-3 px-md-4 mt-4">
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            <h1>Gerenciar Cedentes</h1>
             <form method="GET" action="listar_cedentes.php" class="d-flex ms-auto me-3" style="max-width: 300px;">
                ...
                <input class="form-control me-2" type="search" name="search" placeholder="Buscar cedente..." aria-label="Buscar" value="<?php echo htmlspecialchars($search); ?>">
            ...
            <a href="form_cedente.php" class="btn btn-success">
                <i class="bi bi-plus-lg"></i> Novo Cedente
            </a>
        </div>

        <?php if (isset($_GET['status']) && $_GET['status'] == 'success'): ?>
            <div class="alert alert-success alert-dismissible fade show" role="alert">Cedente salvo com sucesso!<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
        <?php elseif (isset($_GET['status']) && $_GET['status'] == 'deleted'): ?>
             <div class="alert alert-success alert-dismissible fade show" role="alert">Cedente excluído com sucesso!<button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>
        ...
        <?php if (empty($cedentes) && $total_results == 0 && empty($search)): ?>
            <div class="alert alert-info">Nenhum cedente cadastrado ainda. <a href="form_cedente.php">Clique aqui para adicionar</a>.</div>
        <?php elseif (empty($cedentes) && $total_results > 0): ?>
             <p class="alert alert-warning">Nenhum cedente encontrado para a busca: "<?php echo htmlspecialchars($search); ?>" nesta página.</p>
        <?php elseif (empty($cedentes) && $total_results == 0): ?>
            <div class="alert alert-warning">Nenhum cedente encontrado para a busca: "<?php echo htmlspecialchars($search); ?>"</div>
        <?php else: ?>
            <div class="table-responsive">
                <table class="table table-striped table-bordered table-hover">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th><?php echo getSortLink('documento_principal', 'CNPJ/CPF', $sort, $dir, $search); ?></th>
                    ...
                    <tbody>
                        <?php foreach ($cedentes as $cedente): ?>
                            <tr>
                                <td><span class="badge bg-secondary"><?php echo $cedente['id']; ?></span></td>
                                <td><?php echo htmlspecialchars($cedente['empresa'] ?? '-'); ?></td>
                                <td><?php echo htmlspecialchars($cedente['email'] ?? '-'); ?></td>
                                <td><?php echo htmlspecialchars($cedente['telefone'] ?? '-'); ?></td>
                                <td><?php
                                ...
                                <td><?php
                                    ...
                                    echo formatDocumento($cedente['documento_principal'] ?? '', $cedente['tipo_pessoa'] ?? '');
                                ?></td>
                                <td class="text-center">
                                    <a href="visualizar_cedente.php?id=<?php echo $cedente['id']; ?>" class="text-info action-icon" title="Visualizar Cedente"><i class="bi bi-eye-fill"></i></a>
                                    <a href="form_cedente.php?id=<?php echo $cedente['id']; ?>" class="text-primary action-icon" title="Editar Cedente"><i class="bi bi-pencil-square"></i></a>
                                    <a href="excluir_cedente.php?id=<?php echo $cedente['id']; ?>" class="text-danger action-icon delete-btn" title="Excluir Cedente" onclick="return confirm('Tem certeza que deseja excluir este cedente? Operações associadas a ele podem ficar sem referência.');"><i class="bi bi-trash3-fill"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
             ...
             <?php if ($total_pages > 1): ?>
                ...
                <nav aria-label="Paginação Cedentes">
                 ...
                 <p class="text-center text-muted">Página <?php echo $page; ?> de <?php echo $total_pages; ?> (Total: <?php echo $total_results; ?> cedentes)</p>
                 <?php // --- Fim Código Paginação --- ?>
            <?php elseif($total_results > 0): ?>
                 <p class="text-center text-muted">Total: <?php echo $total_results; ?> cedente<?php echo $total_results == 1 ? '' : 's'; ?></p>
            <?php endif; ?>
